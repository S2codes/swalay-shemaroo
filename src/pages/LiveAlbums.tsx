...
const LiveAlbums = () => {
  ...
  const filteredAlbums = useMemo(() => {
    ...
    return albumData.filter((album) => {
      // Only show albums where status is 1 (live)
      if (album.status !== 2) return false;
  ...
  const handleExportAll = async () => {
    try {
      ...
      // Filter albums to only include live ones (status === 2)
      const liveAlbums = albumData.filter(album => album.status === 2);
      ...
      const csvContent = exportAlbumsWithTracksToCSV(liveAlbums, tracksData);
      
      // Download the file
      fileDownload(csvContent, "live_albums_with_tracks.csv");
      ...
      fileDownload(csvContent, "live_albums.csv");
    } finally {
      // Reset button state
      const exportButton = document.querySelector('[data-export-button]') as HTMLButtonElement;
      if (exportButton) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Live Albums</h1>
            <p className="text-gray-600">Albums that are live and available</p>
          </div>
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="relative w-full md:w-64">
                ...
                <Input
                  placeholder="Search live albums..."
                  value={searchTerm}
                      ...
                      <TableCell colSpan={8} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-gray-500">
                            {searchTerm
                              ? "No live albums match your search"
                              : "No live albums found"}
                          </p>
                          {searchTerm && (
                            <Button
                              variant="ghost"
